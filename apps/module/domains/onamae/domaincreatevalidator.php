<?php
class onamae_domaincreatevalidator extends core_api {
	var $array;
	var $query_string;
	var $params;
	var $data;
	...
	public function get ($params) {
		...
		$this->data = array(
			...
			"actionType"=>"DomainCreateValidator",
			"responseFormat"=>domain_api::$responseFormat
		);
		$this->flag_domain = '';
		$this->checkValidateParams();
		return $this->onamae_DomainCreateValidator();
	}
	public function onamae_DomainCreateValidator () {
	...
	public function setParseSingle () {
		...
		if(isset($this->params["srLrKeyword"])&&$this->params["srLrKeyword"]!=='') {
			$this->query_string = $this->query_string."&srLrKeyword=".$this->params["srLrKeyword"];
		}
		// tncId
		if(isset($this->params["tncId"])&&$this->params["tncId"]!=='') {
			$this->query_string = $this->query_string."&tncId=".$this->params["tncId"];
		}
		// smd
		if(isset($this->params["smd"])&&$this->params["smd"]!=='') {
			$this->query_string = $this->query_string."&smd=".$this->params["smd"];
		}
		// RegistryPremium
		if(isset($this->params["RegistryPremium"])&&$this->params["RegistryPremium"]!=='') {
			$this->query_string = $this->query_string."&RegistryPremium=".$this->params["RegistryPremium"];
		}
		// emailForwardTo
	...
	public function setParseBulk () {
		...
		// srLrKeywords
		if(isset($this->params["srLrKeywords"])&&is_array($this->params["srLrKeywords"])) {
			foreach ($this->params["srLrKeywords"] as $srLrKeywords_key => $srLrKeywords_value) {
			 	$this->query_string = $this->query_string."&srLrKeywords=".$srLrKeywords_value;
			}
		}
		// tncIds
		if(isset($this->params["tncIds"])&&is_array($this->params["tncIds"])) {
			foreach ($this->params["tncIds"] as $tncIds_key => $tncIds_value) {
			 	$this->query_string = $this->query_string."&tncIds=".$tncIds_value;
			}
		}
		// smds
		if(isset($this->params["smds"])&&is_array($this->params["smds"])) {
			foreach ($this->params["smds"] as $smds_key => $smds_value) {
			 	$this->query_string = $this->query_string."&smds=".$smds_value;
			}
		}
		// RegistryPremiums
		if(isset($this->params["RegistryPremiums"])&&is_array($this->params["RegistryPremiums"])) {
			foreach ($this->params["RegistryPremiums"] as $RegistryPremiums_key => $RegistryPremiums_value) {
			 	$this->query_string = $this->query_string."&RegistryPremiums=".$RegistryPremiums_value;
			}
		}
		// emailForwardTos
	...
	public function checkValidateParams () {
		...
		if(isset($this->params["domainNames"])&&is_array($this->params["domainNames"])) {
			...
			if(count($this->params["domainNames"])>500) {
				//exit;
			}
		}
		if(isset($this->params["periods"])&&is_array($this->params["periods"])) {
			if(count($this->params["periods"])>500) {
				//exit;
			}
		}
		if(isset($this->params["autoRenews"])&&is_array($this->params["autoRenews"])) {
			if(count($this->params["autoRenews"])>500) {
				//exit;
			}
		}
		if(isset($this->params["proxyflgs"])&&is_array($this->params["proxyflgs"])) {
			if(count($this->params["proxyflgs"])>500) {
				//exit;
			}
		}
		if($this->flag_domain==='') {
		...
		// preRegist
		if(isset($this->params["preRegist"])&&$this->params["preRegist"]!=='') {
			$this->data["preRegist"] = $this->params["preRegist"];
		}
